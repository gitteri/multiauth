...
class AuthPasswordBrokerTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $mocks = $this->getMocks();
        $broker = $this->getMock('Ollieread\Multiauth\Passwords\PasswordBroker', [ 'getUser', 'makeErrorRedirect'], array_values($mocks));
    ...
    {
        $mocks = $this->getMocks();
        $broker = $this->getMock('Ollieread\Multiauth\Passwords\PasswordBroker', ['emailResetLink', 'getUri'], array_values($mocks));
        $mocks['users']->shouldReceive('retrieveByCredentials')->once()->with(['foo'])->andReturn($user = m::mock('Illuminate\Contracts\Auth\CanResetPassword'));
        $mocks['tokens']->shouldReceive('create')->once()->with($user, $mocks['type'])->andReturn('token');
        $callback = function () {};
    ...
    public function testMailerIsCalledWithProperViewTokenAndCallback()
    {
        ...
        $mocks['mailer']->shouldReceive('send')->once()->with('resetLinkView', ['token' => 'token', 'user' => $user, 'type' => $mocks['type']], m::type('Closure'))->andReturnUsing(function ($view, $data, $callback) {
            return $callback;
        });
    ...
    {
        $creds = ['token' => 'token'];
        $broker = $this->getMock('Ollieread\Multiauth\Passwords\PasswordBroker', ['validateNewPassword'], array_values($mocks = $this->getMocks()));
        ...
        $mocks['tokens']->shouldReceive('exists')->with($user, 'token', $mocks['type'])->andReturn(false);
    ...
    {
        unset($_SERVER['__password.reset.test']);
        $broker = $this->getMock('Ollieread\Multiauth\Passwords\PasswordBroker', ['validateReset', 'getPassword', 'getToken'], array_values($mocks = $this->getMocks()));
        ...
        $mocks['tokens']->shouldReceive('delete')->once()->with('token', $mocks['type']);
    ...
    {
        return new \Ollieread\Multiauth\Passwords\PasswordBroker($mocks['type'], $mocks['tokens'], $mocks['users'], $mocks['mailer'], $mocks['view']);
    }
    protected function getMocks()
    {
        $mocks = [
            'type'      => 'multiAuth', //custom auth type
            'tokens'    => m::mock('Ollieread\Multiauth\Passwords\TokenRepositoryInterface'),
