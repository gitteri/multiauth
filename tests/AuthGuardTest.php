...
class AuthGuardTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        list($session, $provider, $name, $request, $cookie) = $this->getMocks();
        $guard = m::mock('Ollieread\Multiauth\Guard[check,attempt]', [$provider, $session, $name]);
    ...
    {
        list($session, $provider, $name, $request, $cookie) = $this->getMocks();
        $guard = m::mock('Ollieread\Multiauth\Guard[check]', [$provider, $session, $name]);
    ...
    {
        list($session, $provider, $name, $request, $cookie) = $this->getMocks();
        $guard = $this->getMock('Ollieread\Multiauth\Guard', ['login'], [$provider, $session, $request]);
        $guard->setDispatcher($events = m::mock('Illuminate\Contracts\Events\Dispatcher'));
        $events->shouldReceive('fire')->once()->with('auth.attempt', [['foo'], false, true]);
    ...
    {
        list($session, $provider, $name, $request, $cookie) = $this->getMocks();
        $mock = $this->getMock('Ollieread\Multiauth\Guard', ['getName'], [$provider, $session, $request]);
    ...
    {
        list($session, $provider, $name, $request, $cookie) = $this->getMocks();
        $mock = $this->getMock('Ollieread\Multiauth\Guard', ['user'], [$provider, $session, $request]);
    ...
    {
        list($session, $provider, $name, $request, $cookie) = $this->getMocks();
        $mock = $this->getMock('Ollieread\Multiauth\Guard', ['getName', 'getRecallerName'], [$provider, $session, $request]);
    ...
    {
        list($session, $provider, $name, $request, $cookie) = $this->getMocks();
        $mock = $this->getMock('Ollieread\Multiauth\Guard', ['clearUserDataFromStorage'], [$provider, $session, $request]);
    ...
    {
        list($session, $provider, $name, $request, $cookie) = $this->getMocks();
        $guard = new Ollieread\Multiauth\Guard($provider, $session, $name, $request);
    ...
    {
        list($session, $provider, $name, $request, $cookie) = $this->getMocks();
        $guard = $this->getMock('Ollieread\Multiauth\Guard', ['login', 'user'], [$provider, $session, $request]);
    ...
    {
        list($session, $provider, $name, $request, $cookie) = $this->getMocks();
        return new Ollieread\Multiauth\Guard($provider, $session, $name, $request);
    }
